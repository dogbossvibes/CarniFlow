...
export async function getTrackSessionById(id: string): Promise<Result<any>> {
  try {
    const [{ data: session, error: sErr }, { data: points }, { data: markers }, { data: runs }, engineRes] = await Promise.all([
      ...
      // 0 Zeilen → ERWARTBARER Zustand, KEIN Fehler (früher PGRST116 → console.error/LogBox).
    ...
    // Echte Fehler (Netzwerk/Auth/RLS/Server) weiterhin eskalieren + loggen; 0-rows nicht.
    if (sErr && (sErr as { code?: string }).code !== 'PGRST116') return fail('getTrackSessionById', sErr);
    // Local-first: keine Remote-Zeile → still „not found" (KEIN console.error). Der Caller
    // (app/track/[id].tsx) baut die Fährte dann vollständig aus SQLite zusammen.
