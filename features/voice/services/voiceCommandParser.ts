...
const RULES: { match: RegExp; cmd: () => VoiceCommand }[] = [
  ...
  { match: /\b(abriss|abriß)\b/, cmd: () => ({ type: 'ADD_MARKER', markerType: 'winkel', label: 'Abriss', angleKind: 'abriss' }) },
  { match: /\b(spitz(winkel)?|spitzer winkel)\s*(nach\s*)?links\b/, cmd: () => ({ type: 'ADD_MARKER', markerType: 'winkel', label: 'Spitzwinkel links', angleKind: 'spitz_links' }) },
  { match: /\b(spitz(winkel)?|spitzer winkel)\s*(nach\s*)?rechts\b/, cmd: () => ({ type: 'ADD_MARKER', markerType: 'winkel', label: 'Spitzwinkel rechts', angleKind: 'spitz_rechts' }) },
