...
-- (NOT EXISTS auf legacy_message_id verhindert Duplikate). Läuft auch
-- fehlerfrei durch, wenn die alte messages-Tabelle gar nicht existiert.

do $$
begin
  if to_regclass('public.messages') is null then
    raise notice 'Tabelle public.messages existiert nicht – nichts zu migrieren.';
    return;
  end if;
  ...
  where not exists (
    select 1 from public.connection_messages cm where cm.legacy_message_id = m.id
  );
end $$;
