...
(function () {
    ...
    return (
      <button onClick={onClick} className="card" style={{ padding: big ? 14 : 12, display: 'flex', alignItems: 'center', gap: 13,
        ...
        <div style={{ width: big ? 58 : 50, height: big ? 58 : 50, borderRadius: 13, overflow: 'hidden', flexShrink: 0,
          ...
          <V.TrackSketch legs={h.angles} objects={h.objects} w={58} h={58} progress={1} showLabels={false}/>
        </div>
  ...
  function CTA({ nav, label = 'Neue Fährte planen' }) {
    return (
      <button className="btn btn-primary" style={{ width: '100%', padding: '17px' }} onClick={() => nav('planen')}>
        <I.plus w={18}/> {label}
      </button>
    );
  }
        ...
        <div className="scroll" style={{ flex: 1, padding: '2px 18px 16px' }}>
          ...
          <div className="card" style={{ padding: 0, overflow: 'hidden', position: 'relative', marginBottom: 14,
            ...
            <div style={{ position: 'absolute', inset: 0, opacity: 0.5 }}>
              <V.TrackSketch legs={4} objects={4} w={340} h={340} progress={0.7} showLabels={false}/>
            </div>
          ...
          <C.SectionLabel action="Details" onAction={() => nav('planen')}>Bedingungen heute</C.SectionLabel>
  ...
  // ════════════════ C · MAP FIRST ════════════════
  function OverviewMap({ dog, dogs, onDog, wx, history, stats, nav }) {
    const last = history[0];
    const actions = [
      { icon: I.plus, l: 'Planen', s: 'planen', p: true },
      { icon: I.play, l: 'Live', s: 'live' },
        ...
        <div className="scroll" style={{ flex: 1, padding: '2px 18px 16px' }}>
          ...
          <div className="card" style={{ padding: 0, overflow: 'hidden', marginBottom: 14, position: 'relative', border: '1px solid rgba(21,230,195,0.16)' }}>
            <div style={{ height: 248 }}>
              <V.GpsMap legs={last.angles} objects={last.objects} progress={1} live={false} w={344} h={248}/>
            </div>
            ...
            {history.slice(0, 4).map((h, i) => (
              <button key={i} onClick={() => nav('auswertung')} style={{ display: 'flex', alignItems: 'center', gap: 14, width: '100%',
                ...
                <div style={{ width: 46, height: 46, borderRadius: 11, overflow: 'hidden', background: '#0a1310', border: '1px solid var(--line)', flexShrink: 0 }}>
                  <V.TrackSketch legs={h.angles} objects={h.objects} w={46} h={46} progress={1} showLabels={false}/>
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
