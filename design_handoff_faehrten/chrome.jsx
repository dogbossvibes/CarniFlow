...
(function () {
  ...
  function BottomNav({ active = 'Fährten' }) {
    const items = [
      { k: 'Start', icon: I.home }, { k: 'Hunde', icon: I.paw },
      { k: 'Fährten', icon: I.route }, { k: 'Hub', icon: I.grid }, { k: 'Profil', icon: I.user },
    ];
    return (
      <div style={{ flexShrink: 0, position: 'relative', paddingBottom: 26, paddingTop: 8 }}>
        <div style={{ position: 'absolute', inset: 0, top: -18,
          background: 'linear-gradient(180deg, rgba(0,0,0,0), #000 38%)' }}/>
        <div className="glass" style={{ position: 'relative', margin: '0 14px', borderRadius: 24, padding: '9px 6px',
          display: 'flex', justifyContent: 'space-around', alignItems: 'center' }}>
          {items.map(it => {
            const on = it.k === active;
            return (
              <div key={it.k} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4,
                color: on ? 'var(--acc)' : 'var(--faint)', position: 'relative', padding: '2px 4px' }}>
                {on && <div style={{ position: 'absolute', top: -9, width: 26, height: 3, borderRadius: 3,
                  background: 'var(--acc)', boxShadow: '0 0 10px var(--acc-glow)' }}/>}
                <it.icon w={21}/>
                <span style={{ fontSize: 9.5, fontWeight: on ? 700 : 600, letterSpacing: '0.02em' }}>{it.k}</span>
              </div>
            );
          })}
        </div>
